...
@db_router.get("/is_maker_exists", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker"])
...
@db_router.get("/is_maker_exists_by_id", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker"])
...
@db_router.post("/add_maker", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker"])
async def add_maker(guild_id: int, discord_id: int, nickname: str):
    new_maker = models.Maker(guild_id=guild_id, discord_id=discord_id, nickname=nickname)
    async with SessionLocal() as session:
...
@db_router.post("/update_maker", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker"])
...
@db_router.get("/get_all_makers", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker"])
async def get_all_makers(guild_id: int):
    async with SessionLocal() as session:
        makers_list = await session.execute(select(models.Maker).filter_by(guild_id=guild_id))
...
@db_router.get("/get_publications_by_maker", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker"])
...
@db_router.post("/add_maker_action", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker Actions"])
...
@db_router.get("/get_maker_actions", dependencies=[Depends(JWTBearer())], tags=["Database", "Maker Actions"])
async def get_makers_actions(maker_id: int):
    async with SessionLocal() as session:
        actions = await session.execute(
            select(models.MakerAction).filter_by(maker_id=maker_id).order_by(models.MakerAction.timestamp.desc())
        )

    return {"status": "ok", "actions": actions.scalars().all()}


@db_router.post("/add_publication", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
async def add_publication(guild_id: int, publication_number: int):
    new_publication = models.Publication(guild_id=guild_id, publication_number=publication_number)
    async with SessionLocal() as session:
        session.add(new_publication)
        await session.commit()
        publication = await session.execute(
            select(models.Publication).filter_by(guild_id=guild_id, publication_number=publication_number))

    return {"status": "ok", "publication": publication.scalar()}


@db_router.post("/update_publication", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
...
@db_router.post("/delete_publication", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
...
@db_router.get("/is_publication_exists", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
...
@db_router.get("/get_publication", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
...
@db_router.get("/get_publication_by_id", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
async def get_publication(id: int):
    async with SessionLocal() as session:
        publication = await session.execute(
            select(models.Publication).filter_by(id=id))

    return {"status": "ok", "publication": publication.scalar()}


@db_router.get("/get_all_publications", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication"])
async def get_publication(guild_id: int):
    async with SessionLocal() as session:
        publications_list = await session.execute(select(models.Publication).filter_by(guild_id=guild_id))

    return {"status": "ok", "publications": publications_list.scalars().all()}


@db_router.post("/add_pub_action", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication Actions"])
...
@db_router.get("/get_pubs_actions", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication Actions"])
async def get_pubs_actions(pub_id: int):
    async with SessionLocal() as session:
        actions = await session.execute(
            select(models.PublicationAction).filter_by(publication_id=pub_id).order_by(
...
@db_router.get("/get_all_pub_actions", dependencies=[Depends(JWTBearer())], tags=["Database", "Publication Actions"])
